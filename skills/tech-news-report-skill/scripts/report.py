    ...
    try:
        ...
        for item in items:
            news_input.append(
                {
                    "title": str(
                        item.get("title") or item.get("headline") or "タイトル未設定"
                    ),
                    "category": str(
                        item.get("category") or item.get("genre") or "トピック"
                    ),
                    "source_category": str(
                        item.get("source_category") or item.get("source_genre") or ""
                    ),
                    "source": str(
                        item.get("source") or item.get("site") or "出典未設定"
                    ),
            ...
            article_input.append(
                {
                    ...
                    "category": str(
                        article.get("category") or article.get("genre") or "技術記事"
                    ),
                    "source_category": str(
                        article.get("source_category")
                        or article.get("source_genre")
                        or ""
                    ),
                    "source": str(
                        article.get("source") or article.get("site") or "出典未設定"
                    ),
                    ...
                    "audience_hint": str(
                        article.get("recommended_for") or article.get("audience") or ""
                    ),
                    "learning_hint": str(
                        article.get("takeaways")
                        or article.get("learning_points")
                        or article.get("impact")
                        or ""
                    ),
                    "use_case_hint": str(
                        article.get("use_case") or article.get("application") or ""
                    ),
                }
            )
            ...
            podcast_input.append(
                {
                    "title": str(
                        podcast.get("title")
                        or podcast.get("episode_title")
                        or "タイトル未設定"
                    ),
                    "category": str(
                        podcast.get("category") or podcast.get("genre") or "Podcast"
                    ),
                    "source_category": str(
                        podcast.get("source_category")
                        or podcast.get("source_genre")
                        or ""
                    ),
                    "source": source,
                    "url": str(podcast.get("url") or ""),
                    "published_at": str(
                        podcast.get("published_at")
                        or podcast.get("date")
                        or podcast.get("published")
                        or ""
                    ),
                    "episode": str(
                        podcast.get("episode")
                        or podcast.get("episode_number")
                        or source
                    ),
                    "raw_summary": str(
                        podcast.get("summary")
                        or podcast.get("description")
                        or podcast.get("content")
                        or ""
                    ),
                    "highlight_hint": str(
                        podcast.get("highlights")
                        or podcast.get("listening_points")
                        or podcast.get("impact")
                        or ""
                    ),
                    "audience_hint": str(
                        podcast.get("recommended_for") or podcast.get("audience") or ""
                    ),
                }
            )
        ...
        if news_input:
            ...
            for index, item in enumerate(news_input, start=1):
                lines.append(
                    f"- {index}. {item['category']}: {item['title']} - {item['source']} ({item['url'] or 'N/A'})"
                )
        if article_input:
            lines.append("\n## articles")
        ...
        metadata = {
            "items": len(news_input),
            "articles": len(article_input),
            "podcasts": len(podcast_input),
            "focus": focus,
        }
        return {
            "ok": True,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="JSON項目を技術ニュースレポート用の素材JSONに正規化します。"
    )
    parser.add_argument("--items-json", help="JSON array of news items")
    parser.add_argument(
        "--items-file", help="Path to a JSON file containing an array of news items"
    )
    parser.add_argument(
        "--articles-json", help="JSON array of recommended technical articles"
    )
    parser.add_argument(
        "--articles-file",
        help="Path to a JSON file containing recommended technical articles",
    )
    parser.add_argument(
        "--podcasts-json", help="JSON array of recommended podcast episodes"
    )
    parser.add_argument(
        "--podcasts-file",
        help="Path to a JSON file containing recommended podcast episodes",
    )
