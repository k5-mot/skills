...
def test_format_news_report_prepares_report_input():
    ...
    assert (
        report_input["articles"][0]["learning_hint"]
        == "Improve internal developer experience."
    )
    assert (
        report_input["articles"][0]["use_case_hint"]
        == "Apply the workflow to service templates."
    )
    assert report_input["podcasts"][0]["category"] == "生成AI"
    assert (
        report_input["podcasts"][0]["highlight_hint"]
        == "Model serving and GPU operations."
    )
