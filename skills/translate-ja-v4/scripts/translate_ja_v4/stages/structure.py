            ...
            texts.append(
                {
                    ...
                    "spans": [
                        {
                            key: span.get(key)
                            for key in ("text", "bbox", "font", "size", "weight")
                        }
                        for span in matching_spans(document, item)
                    ],
                }
            )
    ...
    for patch in patches:
        ...
        if patch.op == "set_label" and target and patch.label in {"code", "caption"}:
            ...
            if (
                ...
                left["label"] = "code"
                right["text"] = ""
                right.setdefault("structure_ja_v4", {})["merged_into"] = self_ref(
...
def _fix_heading_jumps(document: dict[str, Any]) -> int:
    """見出し階層が一度に二段以上深くならないよう補正する。

    Args:
        document: StructureStageで補正中のDocling JSON。

    Returns:
        見出しlevelを変更した要素数。

    Side Effects:
        document内の見出しlevelを直接更新する。
    """
    ...
    for item in document.get("texts", []):
        if not isinstance(item, dict) or item.get("label") not in heading_labels:
            continue
        level = item.get("level")
        if not isinstance(level, int) or not 1 <= level <= 6:
            level = 1 if previous == 0 else min(previous + 1, 6)
        allowed = 1 if previous == 0 else min(previous + 1, 6)
        normalized = min(level, allowed)
        if item.get("level") != normalized:
