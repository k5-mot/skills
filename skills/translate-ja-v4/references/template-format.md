...
目次titleは「目次」、無番号とし、`TOC \\o "1-6" \\h \\z \\u` を基準に本冊の見出し1〜6だけを含める。図目次は `TOC \\h \\z \\c "図"`、表目次は `TOC \\h \\z \\c "表"` とし、captionの `SEQ 図` / `SEQ 表` fieldを参照する。DocxStageが3種類を文書先頭へ自動挿入し、Wordで開いたときにfieldを更新可能にする。TOC field後に古いcache textを重複させない。
...
Markdown上で内容が `---` だけの段落は、Wordでは文字列のまま残さず、段落下罫線による水平線として表示する。
