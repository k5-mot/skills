...
def _reviewed(document: dict[str, Any], target: dict[str, Any]) -> bool:
    ...
    Returns:
        review_ja_v3 metadataがあればTrue。
    """

    meta = resolve_target(document, target["path"]).get("translate_ja_v3", {})
    return "review_ja_v3" in meta
...
def _apply_review(
    ...
    Side Effects:
        translate_ja_v3 metadataを更新する。
    """

    item = resolve_target(document, target["path"])
    meta = item["translate_ja_v3"]
    ...
    meta["review_ja_v3"] = {"reason": result["reason"]}
