...
互換性を優先してstructured outputやJSONを要求せず、レビュー後の日本語だけを受け取る。空応答では元の訳文を保持して続行する。変更できるのは `translate_ja_v2` の訳文と描画値だけであり、原文、Docling構造、順序、label、表構造は変更しない。`--skip-review` ではこの工程を `skipped` と記録し、翻訳済みJSONからMarkdownを生成する。
