...
互換性を優先してstructured outputやJSONを要求せず、レビュー後の日本語だけを受け取る。空応答、異なる原文を持つ隣接要素の訳文と95%以上一致する応答、原訳の1.5倍を超えかつ200文字を超える応答、100文字以上の原訳を60%未満へ短縮する応答、Review入力の不足を訴えるメタ応答、日本語を含む原訳から日本語をすべて除く応答では、モデルが対象を誤認したものとして原訳を保持する。変更できるのは `translate_ja_v2` の訳文と描画値だけであり、原文、Docling構造、順序、label、表構造は変更しない。`--skip-review` ではこの工程を `skipped` と記録し、翻訳済みJSONからMarkdownを生成する。
