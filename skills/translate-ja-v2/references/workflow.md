...
各候補は `--max-batch-elements 0`（既定）では固定の要素数上限を設けず、正数なら指定件数以内に分割する。その後、推定翻訳応答が安全上限12,000文字を超える場合に要素境界で事前分割する。LLM時はさらにsystem prompt、翻訳ルール、共有文脈、共有用語集、入力JSONを含む完成messagesを作り、テキスト全体が `--context-chars` を超える候補も分割する。単一要素でもいずれかの文字数上限を超える場合はAPIを呼ばずに失敗させる。
...
LLM時はタイムアウトなどの一時的なAPI障害が最大6回の試行後も続く場合も、失敗した複数要素バッチを二分して再実行する。入力容量エラー（[判定条件](spec.md#6-固定値)）は同サイズでの再試行をせず二分する。必要なら単一要素まで縮小し、単一要素のAPI障害は未完了のまま停止する。認証・設定エラーは分割しない。子バッチの結果は元IDへ復元し、親バッチ全体の成功後に進捗を保存する。縮小した件数を後続バッチの上限として引き継ぐことはない。
