...
翻訳済み要素を原文と訳文の合計が `--batch-chars` 以内となるバッチへ詰め、最大4バッチを並列実行する。各要素にはID、原文、現在の訳文、見出し文脈、用語集該当語、保護するインラインコード、文書順で確定した直前・直後の訳文を持たせる。APIはID付きJSONでレビュー後の訳文を返し、意味、数量、否定、固有名詞、表記ゆれ、文体を確認する。
