...
texts、表タイトル、自然言語を含む表セルを翻訳する。コードブロック、ページヘッダー、ページフッター、URL、パス、コマンド、識別子、コードだけの表セルは翻訳しない。ページ装飾は原文を描画値として保持する。
