...
texts、表タイトル、自然言語を含む表セルを翻訳する。コードブロック、URL、パス、コマンド、識別子、コードだけの表セルは翻訳しない。

見出しと配下要素を意味ブロックにする。同じレベルまたは上位の見出しで次のブロックを開始し、ブロック単位を保ちながら原文合計を `--batch-chars` 以内へ詰める。上限を超えるブロックは要素境界で分割し、単一要素だけで上限を超える場合はその要素を単独送信する。表はタイトルとセルを同様に詰める。

翻訳APIには各要素IDを渡し、JSON objectの `translations` で同じID集合を返させる。IDの欠落、追加、変更、重複、空訳は採用しない。空応答または部分応答で複数要素がある場合は要素境界で二分して再実行する。
...
翻訳済み要素を1件ずつ独立したrequestでレビューし、最大4件を並列実行する。各requestには原文、現在の訳文、見出し文脈、用語集該当語、保護するインラインコード、文書順で確定した直前・直後の訳文を渡し、意味、数量、否定、固有名詞、表記ゆれ、文体を確認する。完了順に要素単位の結果と進捗を保存するため、中断後は未完了要素だけを再開する。

互換性を優先してstructured outputやJSONを要求せず、レビュー後の日本語だけを受け取る。空応答、異なる原文を持つ隣接要素の訳文と95%以上一致する応答、原訳の1.5倍を超えかつ200文字を超える応答では、モデルが対象を誤認したものとして原訳を保持する。変更できるのは `translate_ja_v2` の訳文と描画値だけであり、原文、Docling構造、順序、label、表構造は変更しない。`--skip-review` ではこの工程を `skipped` と記録し、翻訳済みJSONからMarkdownを生成する。
