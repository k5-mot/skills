...
LLM には structured output や JSON 形式を要求せず、文書順に1要素ずつ通常テキストとしてレビュー後の日本語訳だけを返させる。各要素の直前に前後の最新訳文を参照し、先に補正した表記を後続要素のレビューへ反映する。応答が空の場合は、その要素だけ元訳を使う。訳文を変更した場合は、対象IDと変更文字数をDEBUGログへ出力する。見出しと表タイトルはレビュー後も `英語 / 日本語` の render text を保つ。
