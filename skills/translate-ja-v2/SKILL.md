...
Docling 変換は常に `/v1/convert/file/async` を使い、polling ごとに poll 回数と status をDEBUG loggingする。ログ本文は英語、アプリの既定レベルはDEBUG、依存ライブラリはWARNING以上とし、各レベル名をANSI色付きで表示する。工程の開始・完了・省略・ResumeはINFOにする。
