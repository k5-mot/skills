...
レビュー対象は翻訳済みの本文、見出し、表タイトル、表セルです。コード、URL、パス、識別子など翻訳対象外の要素は含めません。レビュー応答も入力IDとの完全一致を検証し、欠落、追加、変更、重複があればその batch を分割して再実行します。単一要素まで分割してもレビュー応答が空または不正な場合は、その要素だけ元の訳文を使って継続します。
