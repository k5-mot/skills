...
def test_list_channels_calls_get(monkeypatch):
    ...
    fake.get_queue.append(
        FakeResponse(200, {"channels": [{"id": "ch1", "name": "daily-report"}]})
    )
...
def test_resolve_channel_id_from_name(monkeypatch):
    ...
    fake = FakeRequests()
    fake.get_queue.append(
        FakeResponse(200, [{"id": "ch_daily", "name": "daily-report"}])
    )
    monkeypatch.setattr(client, "requests", fake)
...
def test_resolve_missing_channel_returns_false(monkeypatch):
    ...
    fake = FakeRequests()
    fake.get_queue.append(
        FakeResponse(200, {"channels": [{"id": "ch1", "name": "other"}]})
    )
    monkeypatch.setattr(client, "requests", fake)
...
def test_post_message_calls_post(monkeypatch):
    ...
    fake = FakeRequests()
    fake.get_queue.append(
        FakeResponse(200, {"channels": [{"id": "ch_daily", "name": "daily-report"}]})
    )
    fake.post_queue.append(FakeResponse(200, {"id": "msg1"}))
    monkeypatch.setattr(client, "requests", fake)

    result = client.post_message(
        "#daily-report", "## Report", metadata={"report_type": "daily"}
    )
    ...
    assert (
        post_call[1]
        == "https://openwebui.example.test/api/v1/channels/ch_daily/messages/post"
    )
...
def test_post_message_to_thread_sets_parent_id(monkeypatch):
    ...
    result = client.post_message(
        "channel-id-1234567890", "thread reply", thread_id="msg-parent"
    )
...
def test_list_channel_messages_calls_get(monkeypatch):
    ...
    fake.get_queue.append(
        FakeResponse(200, [{"id": "msg1", "content": "hello", "created_at": 1}])
    )
    monkeypatch.setattr(client, "requests", fake)
    ...
    assert (
        fake.calls[1][1]
        == "https://openwebui.example.test/api/v1/channels/ch_daily/messages?skip=0&limit=10"
    )
...
def test_list_all_channel_messages_pages_until_short_page(monkeypatch):
    ...
    assert (
        fake.calls[0][1]
        == "https://openwebui.example.test/api/v1/channels/channel-id-1234567890/messages?skip=0&limit=2"
    )
    assert (
        fake.calls[1][1]
        == "https://openwebui.example.test/api/v1/channels/channel-id-1234567890/messages?skip=2&limit=2"
    )
...
def test_get_file_content_calls_data_content(monkeypatch):
    ...
    assert (
        fake.calls[0][1]
        == "https://openwebui.example.test/api/v1/files/file1/data/content"
    )
...
def test_list_all_chats_pages_until_short_page(monkeypatch):
    ...
    fake = FakeRequests()
    fake.get_queue.append(
        FakeResponse(200, {"chats": [{"id": "chat1"}, {"id": "chat2"}]})
    )
    ...
    assert (
        fake.calls[0][1]
        == "https://openwebui.example.test/api/v1/chats?skip=0&limit=2&include_archived=False"
    )
    assert (
        fake.calls[1][1]
        == "https://openwebui.example.test/api/v1/chats?skip=2&limit=2&include_archived=False"
    )
...
def test_add_user_message_to_chat_updates_then_completes(monkeypatch):
    ...
    result = client.add_user_message_to_chat(
        "chat1", "next question", "llama3.2", stream=False
    )
    ...
    assert (
        update_call[4]["chat"]["history"]["currentId"] == result["assistant_message_id"]
    )
...
def test_post_endpoint_template_can_be_overridden(monkeypatch):
    ...
    set_required_env(monkeypatch)
    monkeypatch.setenv(
        "OPEN_WEBUI_CHANNELS_POST_PATH_TEMPLATE", "/api/channels/{channel_id}/posts"
    )
    fake = FakeRequests()
    ...
    assert (
        fake.calls[0][1]
        == "https://openwebui.example.test/api/channels/channel-id-1234567890/posts"
    )
