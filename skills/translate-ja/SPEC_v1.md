...
JSON 内の `image.uri` は `artifacts/...png` のような相対パスとして扱い、`source.bronze.json` の親ディレクトリを基準に解決する。出力画像directoryはCLIの`--artifacts-path`で指定しない。
...
Pythonはプロジェクトの`.venv/bin/python3`が存在する場合はそれを優先し、なければ`python3`を使う。
