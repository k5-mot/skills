...
- Structure resolves the exact page image from `pages[].image.uri` and sends only that image with the page's text JSON. A missing URI or file falls back to text-only input, never to an unrelated image. If the page text context exceeds `--context-chars` (50,000 by default), it falls back to adjacent two-element merge checks, then all-pairs two-element order checks with the same page image.
