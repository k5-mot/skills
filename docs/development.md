...
- Translate batches heading/body sections up to 3,000 source characters and batches each table when it fits. Code and protected table cells are excluded. Batch response IDs must exactly match request IDs before translations are applied. Optional CSV glossary entries are filtered per source text before being passed to the LLM with translation rules.
- translate-ja-v2 timeouts, OCR settings, OpenAI retry settings, OpenAI text context limit, translation batch limit, VLM image limit, and log level are fixed constants in `scripts/translate.py`, not environment variables.
