...
- Review checks translated text against neighboring translated elements for mistranslation, glossary conflicts, terminology drift, and nearby style drift. It requests one plain-text reviewed translation per item without structured output or JSON response mode, may update only `translate_ja_v2` translation/render fields, logs the item ID and changed character count when applying a change, keeps the original translation if the response is empty, writes `document.reviewed.json`, and is skipped only by `--skip-review`.
- Multi-agent Review uses Qdrant batch-query evidence, runs independent fidelity and terminology reviewers, and invokes an adjudicator only for differing proposals. Retrieved text is prompt-only; output metadata stores source IDs, locators, scores, proposals, and the decision. `--review-rag` is valid only with `--review-mode multi`.
