...
想定modelはQwen/Gemma系ですが、model名は固定していません。Word入力は初版対象外です。JSON Schema応答はJSON objectのほか、local modelが返す外側の`json`code fenceも受理します。非JSONまたは必須キー不足の応答は、Schemaを明記した修正promptで一度だけ再生成します。
